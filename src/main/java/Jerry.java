...
public class Jerry {
    ...
    private static void loadTasks(ArrayList<Task> tasks) {
        ...
        try (Scanner s = new Scanner(f)) {
            while (s.hasNext()) {
                ...
                if (task != null) {
                    if (p[1].equals("1")) task.markAsDone();
                    tasks.add(task);
                }
            }
        } catch (Exception e) {
    ...
    private static void updateTaskStatus(String input, ArrayList<Task> tasks, boolean isMark) throws JerryException {
        ...
        if (parts.length < 2) throw new JerryException("Please specify a task number.");
    ...
    private static void addTodo(String input, ArrayList<Task> tasks) throws JerryException {
        ...
        if (description.isEmpty()) throw new JerryException("The description of a todo cannot be empty.");
    ...
    private static void addDeadline(String input, ArrayList<Task> tasks) throws JerryException {
        if (!input.contains(" /by ")) throw new JerryException("A deadline must contain ' /by '.");
        String[] parts = input.substring(COMMAND_DEADLINE.length()).split(" /by ", 2);
        if (parts[0].trim().isEmpty()) throw new JerryException("The description of a deadline cannot be empty.");
    ...
    private static void addEvent(String input, ArrayList<Task> tasks) throws JerryException {
        if (!input.contains(" /from ") || !input.contains(" /to ")) throw new JerryException("An event must contain ' /from ' and ' /to '.");
        String[] parts = input.substring(COMMAND_EVENT.length()).split(" /from | /to ", 3);
        if (parts[0].trim().isEmpty()) throw new JerryException("The description of an event cannot be empty.");
    ...
    private static void deleteTask(String input, ArrayList<Task> tasks) throws JerryException {
        String[] parts = input.split(" ");
        if (parts.length < 2) throw new JerryException("Please specify which task number to delete.");
        int index = Integer.parseInt(parts[1]) - 1;
        if (index < 0 || index >= tasks.size()) {
